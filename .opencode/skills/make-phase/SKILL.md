...
Use `command` only for deterministic shell command blocks that the runner may execute directly. Use `verify` for verification command blocks. Use `agent` for work that Codex/OpenCode must perform.
